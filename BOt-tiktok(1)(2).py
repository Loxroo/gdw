...
def send_welcome(message):
    ...
    if iddu in myadmin:
        ...
        D = types.InlineKeyboardButton('قناة المطور', url='https://t.me/vv1ck')
        mas.add(D)
        fg = bot.send_message((message.chat.id), f"هلا عزيزي المشترك “ {nam} ”\n\tفي بوت تشكير تيك توك\n\tاضغط لتشغيل “ /sta  ”", reply_markup=mas)
    else:
        mas = types.InlineKeyboardMarkup(row_width=1)
        A = types.InlineKeyboardButton('المطور', url='https://t.me/vv1ck')
        C = types.InlineKeyboardButton('قناة المطور', url='https://t.me/vv1ck')
        mas.add(A, C)
...
def send_welcome(message):
    ...
    if message.text == '/sta' or message.reply_to_message:
        if message.text == '/sta':
            bot.send_message(message.chat.id, 'ادخل السشن ايدي 🆔')
        if message.reply_to_message:
            mes = message.reply_to_message.text
            if mes == 'ادخل السشن ايدي 🆔':
    ...
    if message.reply_to_message:
        ...
        if mes == 'Number of username required':
            try:
                ...
                A = types.InlineKeyboardButton('بدء ⏸️', callback_data='F1')
                mas.add(A)
...
def sdd(call):
    if call.data == 'F1':
        ...
        for y in range(t):
            ...
            url = 'https://www.tiktok.com/api/uniqueid/check/?region=SA&aid=1233&unique_id=' + ruks + '&app_language=en'
            ...
            try:
                ...
                bot.edit_message_text(chat_id=(call.message.chat.id), message_id=(call.message.message_id), text=f"بدء الصيد ▶️\n\n🆔 ⋮ [{sess}]\n\n≋ Required number ⋮ {t}\n\n≋ \u2066اليوزر ⋮ {ruks}\n≋ \u2066متاح ⋮ {good}\n≋ \u2066مستخدم ⋮ {bad}\n≋ \u2066خطأ ⋮ {error}\n\n≋ عدد اليوزرات ⋮ {kol} ")
